...
8. 以原 Venera 为对照，完成全 App 功能、页面逻辑与设计的系统性对齐；发布前在三个维度分别达到至少 90% 的验收门槛。
...
- 逐像素复制 Flutter 布局或复刻与 Android 不适用的系统交互细节；此项不改变全 App 三个维度分别达到至少 90% 的最终目标。
...
- 首页体验需对照原 Venera 首页设计：先核对其分区、导航关系、卡片层级和关键交互，再用 Android 原生组件重现相近的信息架构与视觉语言，并纳入全 App 设计相似度核验；不要求逐像素复制 Flutter 布局。
- 首页包括最近阅读、收藏更新、本地漫画入口与来源探索内容，并完整呈现加载、空内容、失败及重试状态。
- 最近阅读、收藏更新和本地漫画入口。
- 按来源展示探索页。
- 支持多分区、分页列表和混合布局。
- 分类筛选、排行和查看更多。
...
- 日常功能切片只要求受影响模块和 App 编译通过，不默认运行完整 Lint、测试、实机或压力验证。
- 完整质量门禁只在阶段退出、Release Candidate、专项验证任务或用户明确要求时执行。
- 关键节点要求编译、Lint、单元测试通过。
- 新业务逻辑包含对应测试。
