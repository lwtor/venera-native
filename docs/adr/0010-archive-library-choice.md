...
在 `:core:archive` 内统一使用 Apache Commons Compress 1.28.0，7z LZMA/LZMA2 可选解码依赖 XZ for Java 1.10。输入通过独占 `SeekableByteChannel` 传入，调用方负责关闭 reader；`:core:archive` 是唯一直接依赖 Commons Compress 的模块。归档页索引在导入时写入 Room；读取条目时不重新枚举索引，页面字节经有界 LRU 缓存物化。
