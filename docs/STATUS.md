...
用户明确最终目标是整个 App 与原 Venera 在功能、页面逻辑、设计三个维度分别达到至少 90% 相似度/覆盖度。已写入 `docs/PROJECT_PLAN.md` 产品目标，并新增 `docs/IMPLEMENTATION_PLAN.md` 的 **S4-09 全 App 对照审计与差距收敛**（TODO）；首页专项为 S4-08（TODO）。S4-09 需先冻结上游基线并逐项记录适用范围、权重和证据，三维度分别计算且各自达到 ≥90%，不得用平均分抵消短板。此长期目标不改变当前唯一执行任务 S2-07C，也不代表当前实现已达标。
